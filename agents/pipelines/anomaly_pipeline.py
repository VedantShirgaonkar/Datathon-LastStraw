...
def _get_pipeline_llm(temperature: float = 0.2):
    """Get LLM for anomaly pipeline using centralized routing."""
...
def _get_current_model_name() -> str:
    ...
    return config.featherless.model_primary
...
def detect_anomalies_node(state: AnomalyState) -> dict:
    ...
    llm = _get_pipeline_llm(temperature=0.2)
...
"""
    ...
    try:
        log_llm_call(logger, _get_current_model_name(), prompt_preview=prompt[:200])
...
def investigate_node(state: AnomalyState) -> dict:
    ...
    llm = _get_pipeline_llm(temperature=0.3)
...
def evaluate_alert_node(state: AnomalyState) -> dict:
    ...
    llm = _get_pipeline_llm(temperature=0.1)
