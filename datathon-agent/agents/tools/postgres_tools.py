...
_EMPLOYEE_COLS = (
    "e.id, e.full_name, e.email, e.title, e.role, e.hourly_rate, "
    "e.level, e.location, e.active"
)
_EMPLOYEE_WITH_TEAM = (
...
def get_developer(
    ...
    try:
        ...
        if developer_id:
            ...
            query = f"{base} WHERE e.full_name ILIKE %s"
            params = (f"%{name}%",)
        else:
...
def list_developers(
    ...
    try:
        ...
        if role:
            query += " AND (e.role ILIKE %s OR e.title ILIKE %s)"
            params.extend([f"%{role}%", f"%{role}%"])

        query += f" ORDER BY e.full_name LIMIT %s"
        params.append(limit)
...
def get_project(
    ...
    try:
        ...
        if results:
            ...
            dev_query = """
                SELECT e.id, e.full_name, e.title, e.role,
...
def get_team(
    ...
    try:
        ...
        if results:
            ...
            members_query = """
                SELECT id, full_name, email, title, role, hourly_rate, level, active
