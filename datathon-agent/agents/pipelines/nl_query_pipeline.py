...
def _get_llm(model_key: str = "primary", temperature: float = 0.0):
    """Create an LLM instance using the configured model."""
    config = get_config()
    model_map = {
        "primary": config.featherless.model_primary,
        "code": config.featherless.model_code,
        "fast": config.featherless.model_fast,
        "analytics": config.featherless.model_analytics,
    }
    model = model_map.get(model_key, config.featherless.model_primary)
    return ChatOpenAI(
        model=model,
