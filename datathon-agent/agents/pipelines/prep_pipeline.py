...
def resolve_developer_node(state: PrepState) -> dict:
    ...
    try:
        ...
        rows = pg.execute_query("""
            SELECT e.id, e.full_name, e.email, e.role, e.title, e.hourly_rate,
                   e.level, e.location, e.timezone, e.employment_type,
                   e.start_date, e.active, t.name AS team_name
            FROM employees e
            LEFT JOIN teams t ON e.team_id = t.id
            WHERE e.full_name ILIKE %s OR e.email ILIKE %s
            LIMIT 1
        """, (f"%{name}%", f"%{name}%"))
        ...
        logger.info(f"Resolved: {dev.get('full_name')} ({dev.get('title')}), "
                     f"{len(assignments)} project assignments")
        ...
        return {"developer_info": {}, "status": "error"}
...
def synthesize_briefing_node(state: PrepState) -> dict:
    ...
    if state.get("status") == "error":
        return {
            "briefing": f"❌ Error preparing briefing for '{state['developer_name']}'.",
            "talking_points": [],
        }
    ...
    config = get_config()
    # Use the configured primary model for high-quality briefing synthesis
    _BRIEFING_MODEL = config.featherless.model_primary
    llm = ChatOpenAI(
