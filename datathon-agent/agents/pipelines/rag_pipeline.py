...
def grade_documents_node(state: RAGState) -> dict:
    ...
    grader_llm = ChatOpenAI(
        model=config.featherless.model_fast,  # Hermes 3 8B — fast classifier
...
def generate_answer_node(state: RAGState) -> dict:
    ...
    generator = ChatOpenAI(
        model=config.featherless.model_primary,  # Qwen 72B
        api_key=config.featherless.api_key,
        base_url=config.featherless.base_url,
        temperature=0.1,
    )
    ...
    log_llm_call(logger, model="Qwen-72B (generator)", prompt_preview=query[:80])
...
def hallucination_check_node(state: RAGState) -> dict:
    ...
    config = get_config()
    checker = ChatOpenAI(
        model=config.featherless.model_fast,  # Hermes 3 8B
        api_key=config.featherless.api_key,
        base_url=config.featherless.base_url,
        temperature=0,
    )
    ...
    log_llm_call(logger, model="Hermes-3-8B (hallucination-check)", prompt_preview=answer[:80])
